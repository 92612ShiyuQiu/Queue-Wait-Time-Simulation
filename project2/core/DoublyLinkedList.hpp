...
public:
    ...
    protected:
        Node* itpointer;
        DoublyLinkedList<ValueType> dbl;
        bool pstart;
        bool pend;
...
private:
    ...
    struct Node
    {
        ...
        static Node* copy(const Node* headnd)
        {
            return headnd == nullptr ? nullptr : new Node(headnd->value, headnd->prev, copy(headnd->next));
        }
        static void deleteNode(Node* headnd)
        {
            if (headnd != nullptr)
            {
                deleteNode(headnd->next);
                delete headnd;
            }
        }
...
{
    DoublyLinkedList<ValueType> templist(*this);
    Node* node = head;
    Node* temp = nullptr;
...
{
    Node* toadd = new Node(value, nullptr, nullptr);
    if (this->isEmpty())
    {
        head = toadd;
        tail = toadd;
...
{
    if (pend) {throw IteratorException();}
    else
    {
        if (itpointer->next == nullptr)
        {
            itpointer = nullptr;
            pend = true;
        }
...
{
    if (pstart) {throw IteratorException();}
    else
    {
        if (itpointer->prev == nullptr)
        {
            itpointer = nullptr;
            pstart = true;
        }
...
{
    if (this->pstart == true || this->pend == true) { throw IteratorException();}
    return this->itpointer->value;
    // Note that this is an awful thing I'm doing here, but I needed
    // something that would make this code compile.  You're definitely
    // going to want to replace this with something else before you
    // ever call this member function and expect it to work.  This
...
{
    if (this->pstart == true) {throw IteratorException();}
    else
    {
        this->itpointer = new Node(value, this->itpointer->prev, this->itpointer);
        this->itpointer->prev->next = this->itpointer;
        this->itpointer->next->prev = this->itpointer;
        this->dbl.length += 1;
    }
}
...
{
    if (this->pend == true) {throw IteratorException();}
    else
    {
        this->itpointer = new Node(value, this->itpointer, this->itpointer->next);
        this->itpointer->next->prev = this->itpointer;
        this->itpointer->prev->next = this->itpointer;
        this->dbl.length += 1;
    }
...
{
    if (this->pstart || this->pend) {throw IteratorException();}
    if (moveToNextAfterward)
    {
        this->itpointer = this->itpointer->next;
        this->itpointer->prev = this->itpointer->prev->prev;
        delete this->itpointer->prev->next;
        this->itpointer->prev->next = this->itpointer;
        this->dbl.length -= 1;
    }
    else
    {
        this->itpointer = this->itpointer->prev;
        this->itpointer->next = this->itpointer->next->next;
        delete this->itpointer->next->prev;
        this->itpointer->next->prev = this->itpointer;
        this->dbl.length -= 1;
    }
}
